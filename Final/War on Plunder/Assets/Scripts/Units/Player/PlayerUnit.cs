...
namespace SG.WOP.Units.Player
{
    //Makes sure that the object that this script is
    //attached to will have a NavMeshAgent
    ...
    public class PlayerUnit : MonoBehaviour
    {
        ...
        //As soon as the game starts, there is a reference
        //to NavMeshAgent
        private void Start()
        {
        ...
        private void CheckForEnemyTargets()
        {
            ...
            {
                // Checks if the object that entered the aggro sphere is an enemy target 
        ...
        private void MoveToAggroTarget()
        {
            if (aggroTarget == null)
            {
                // If there is no target then it stays in position
                navAgent.SetDestination(transform.position);
                hasAggro = false;
            }
            else
            {
                // Sets the distance between the unit and the aggro target
